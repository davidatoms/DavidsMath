...
noncomputable def CorrelationFunction {X : Type*} [MeasureTheory.MeasureSpace X]
    (O : X → ℝ) (S : X → ℝ) : ℝ :=
...
def ContinuumLimit (G : Type*) [GaugeGroup G] 
  ...
  sorry  -- limit as lattice_spacing → 0 exists
